...
class CalendarAPI extends ControllerBase {
	...
	public function getEvent() {
		...
		$query = "SELECT
   					a.ID,
   					case
       					when ((select preferenceInd from site_specific_value where orgID = $orgID) = 1) then a.locationTypeID
       					when ((select preferenceInd from site_specific_value where orgID = $orgID) = 2) then a.staffID
       					when ((select preferenceInd from site_specific_value where orgID = $orgID) = 3) then av.dimensionID
   					end as cid,
   					a.orgID,
   					a.actDtlStime as start,
   					a.actDtlEtime as end,
					concat(cv.firstName, ',', cv.lastName) as leader,
   					concat('<b>', av.activityName ,'</b><br><i>',  concat(cv.firstName, ' ', cv.lastName) ,'</i><br> ',  loc.locationName) as title,
				...
				JOIN 
					location_value loc on a.locationTypeID = loc.ID
				JOIN
					contact_value cv on a.staffID = cv.ID
				WHERE 
					a.orgID = $orgID
				GROUP BY 
					a.ID";
